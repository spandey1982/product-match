/**
 * Shared Gemini REST helper for the AI Presenter Reel's script generation.
 *
 * Byte-for-byte the same shape as lib/catalogue-motion/gemini-client.ts
 * (which itself mirrors lib/fashion-designer/gemini-client.ts) — kept as
 * its own small per-domain copy rather than imported, per this repo's
 * established convention (a small local Gemini client per feature domain,
 * not one shared cross-domain client — see that file's own header comment
 * for the fuller rationale and the other examples).
 */
