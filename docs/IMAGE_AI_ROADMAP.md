...
>
> R&D/benchmark findings that became real production decisions are folded
> into `PROJECT_KNOWLEDGE.md` once confirmed — check there for prior findings
> before re-running an experiment. That work happens on the local-only
> `rnd/image-pipeline-benchmarks` branch (never pushed); only approved
> results land here.
>
