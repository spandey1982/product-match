# Home Material Intelligence — Changelog

Part of the domain-scoped docs split approved 2026-09-10 — see
`README.md` for the index. This is the full chronological record of what
shipped, when, why, and how it was tested — moved here verbatim from the
original monolithic `README.md` so the static reference docs
(`product/overview.md`, `domain/data-model.md`, `ai/boundaries.md`,
`architecture/system.md`, `ui/decisions.md`) can stay short and current
...
- **Room-photo privacy policy is still an open gap** (see
  `product/overview.md`) — now more urgent, not less, since real photos of
  real homes are about to be used for actual customer demos.
...
- `GET /api/home-material/materials` — deliberately the **one public,
  unauthenticated** Home Material endpoint (before the 2026-09-10 browse
  page — see `ui/decisions.md` — made products public too). This is
  general reference content, not user- or room-specific, and per brief §8
  Mode B understanding material types logically precedes needing an
  account at all.
...
B. C, D, F remained paused at the time, pending further explicit
go-ahead — **A, C, F shipped 2026-09-10, D permanently deferred** (see
that section below).
...
guarantee (that was C, shipped 2026-09-10). `app/api/home-material/rooms/
...
awareness yet (that was A/C, shipped 2026-09-10).
...
**Adjacency is deliberately manual** — the user-confirmed fact only,
set via the `PATCH .../surfaces/adjacency` endpoint (2+ ids → shared
new group; 1 id → removed from any group) and RoomView.tsx's
`AdjacencyMarker`, shown only when 2+ confirmed walls share the same
repeat_sheet product selection. As of 2026-09-10, an AI SUGGESTION layer
exists on top of this (sub-problem A, see below) — but it still only
ever suggests, never writes `adjacencyGroupId` itself.
...
existing behavior for that specific combination.
...
permanently deferred**, with **UI visual polish next** after this (see
`ui/decisions.md` for what came of that "next").
