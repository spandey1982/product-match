...
export function ModelStudioView({ initialProfiles, faceLibrary }: Props) {
  ...
  const atCap = profiles.length >= MAX_SIGNATURE_MODELS;
  const canCreate = hasAnyFaces && !atCap;
  const addDisabledReason = !hasAnyFaces
    ? "No face references available yet"
    : atCap
    ? `Maximum ${MAX_SIGNATURE_MODELS} Signature Models — delete or edit an existing one to make room`
    : undefined;
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      <section>
        <div className="flex items-baseline justify-between mb-3">
          <h2 className="text-sm font-semibold text-gray-700">Signature Models</h2>
          <span className="text-xs text-gray-400 tabular-nums">
            {profiles.length} / {MAX_SIGNATURE_MODELS}
          </span>
        </div>
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4">
          ...
          >
            ...
            <span className="text-xs font-medium text-center px-2 leading-tight">
              {atCap ? "Cap reached" : "Add Signature Model"}
            </span>
          </button>
        </div>
      </section>
