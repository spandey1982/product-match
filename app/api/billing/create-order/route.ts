...
import { createRazorpayOrder, isRazorpayConfigured, CREDIT_PACKS } from "@/lib/billing/razorpay";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.packId) {
    ...
    } else if (typeof body.customAmountInr === "number") {
      ...
      return NextResponse.json({ error: "packId or customAmountInr is required" }, { status: 400 });
    }

    const rzOrder = await createRazorpayOrder({ amountInr, userId: session.id, packLabel });
